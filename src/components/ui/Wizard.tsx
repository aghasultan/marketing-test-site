...
const WizardField = ({
  ...
  const baseInputStyles = clsx(
    "w-full bg-slate-800/50 border rounded-lg p-4 text-white placeholder-slate-500 focus:ring-2 outline-none transition-all",
    type !== "textarea" && "pr-12", // Padding for icons (except textarea)
  ...
  return (
    <div className="form-group relative">
      <label className="block text-sm font-semibold uppercase tracking-wider text-slate-300 mb-2">{label}</label>
      <div className="relative">
        {type === 'select' ? (
          <select {...register} className={baseInputStyles}>
          ...
          <textarea {...register} rows={rows || 5} className={clsx(baseInputStyles, "resize-none")} placeholder={placeholder} />
        ) : (
          <input type={type} {...register} className={baseInputStyles} placeholder={placeholder} />
        )}

        {/* Validation Icons (Not for textarea) */}
        {type !== 'textarea' && (
...
export const Wizard = () => {
  ...
  const nextStep = async () => {
    ...
    const output = await trigger(fields as unknown as (keyof FormData)[]);
    if (output) {
      setDirection(1);
      setStep((prev) => prev + 1);
    } else {
  ...
  // Handle Form Submission (Enter Key Routing)
  const handleFormSubmit = async (e: React.FormEvent) => {
    e.preventDefault(); // Prevent default browser submit

    if (step < steps.length - 1) {
      // If not last step, try to go next
      await nextStep();
    } else {
      // If last step, run full submit handler
  ...
  return (
    <motion.div
      className="relative backdrop-blur-xl bg-slate-900/40 border border-white/10 rounded-2xl shadow-2xl p-8 max-w-2xl mx-auto overflow-hidden min-h-[600px]"
    ...
    >
      ...
      {isSuccess ? (
        <div className="h-full flex flex-col items-center justify-center p-8 text-center animate-in fade-in duration-500" style={{ minHeight: '530px' }}>
          <motion.div
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
          ...
          <p className="text-slate-300 text-lg max-w-md mx-auto leading-relaxed">
            I'll review your details and get back to you within 24 hours. Keep an eye on your inbox.
          </p>
        </div>
      ) : (
        <>
              ...
              {step === 1 && (
                ...
                >
                  <WizardField
                    ...
                    type="select"
                    register={register('monthlySpend')}
                    error={errors.monthlySpend}
                    isValid={isFieldValid('monthlySpend')}
                  ...
                  <WizardField
                    ...
                    isValid={isFieldValid('primaryGoal')}
                    placeholder="Select goal..."
                    options={[
                      { value: "scale", label: "Aggressive Scale (ROAS > 2)" },
                      { value: "tracking", label: "Fix Tracking & Attribution" },
                      { value: "full_funnel", label: "Full Funnel Buildout" }
                    ]}
                  />
                </motion.fieldset>
              )}
